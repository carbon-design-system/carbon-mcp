...
<a href="https://mcp.carbondesignsystem.com/mcp/auth/ibmid/web" target="_blank">
...
<UnorderedList>
  <ListItem>Login to IBM Cloud</ListItem>
  <ListItem>Complete the brief request form</ListItem>
  <ListItem>
...
## Step 3: Connecting to common MCP clients
...
<UnorderedList>
  <ListItem>Edit the Claude Code configuration file if it exists, else create the file. For macOS/Linux, the configuration file is located `~/.config/claude-code/config.json`</ListItem>
  <ListItem>Insert the following configuration into your config.json file:</ListItem>
</UnorderedList>
...
<UnorderedList>
  <ListItem>Replace `<TOKEN>` and `<SESSION>` with your actual Carbon MCP auth credentials</ListItem>
  <ListItem>Save and close the file</ListItem>
  <ListItem>Quit and relaunch Claude Code</ListItem>
  <ListItem>Verify that Claude Code recognizes Carbon MCP by running this command: <br/> `claude-code --list-mcp-servers`</ListItem>
  <ListItem>You should see Carbon MCP listed:</ListItem>
</UnorderedList>
...
<UnorderedList>
  <ListItem>Open Settings > Extensions</ListItem>
  <ListItem>Click on Advanced settings</ListItem>
  <ListItem>Click Install Extension</ListItem>
  <ListItem>Install the `Carbon MCP Claude Desktop extension`, which can be downloaded [here](./files/carbon-mcp-claude-extension.mcpb).</ListItem>
  <ListItem>Enter your Carbon MCP auth token and session ID in the modal</ListItem>
  <ListItem>Enable the extension</ListItem>
...
<UnorderedList>
  <ListItem>Open Settings > Connectors</ListItem>
  <ListItem>Click on Add Custom Connector</ListItem>
  <ListItem>When prompted for a name, enter `carbon-mcp`</ListItem>
  <ListItem>Under the Remote MCP server URL input, use `https://mcp.carbondesignsystem.com/mcp`</ListItem>
...
<UnorderedList>
  <ListItem>Open the command palette</ListItem>
  <ListItem>Select "View: Open MCP Settings"</ListItem>
  <ListItem>Click on "Add custom MCP"</ListItem>
  <ListItem>It will open a file with your MCP servers where you can add the following configuration:</ListItem>
</UnorderedList>
...
<UnorderedList>
  <ListItem>Replace `<TOKEN>` and `<SESSION>` with your actual Carbon MCP auth credentials</ListItem>
  <ListItem>Save and close the file</ListItem>
  <ListItem>Quit and relaunch Cursor</ListItem>
</UnorderedList>
...
<UnorderedList>
  <ListItem>Open your repository in GitHub</ListItem>
  <ListItem>Go to Settings</ListItem>
  <ListItem>Open Copilot > Coding agent</ListItem>
  <ListItem>Edit the MCP configuration</ListItem>
</UnorderedList>
...
<UnorderedList>
  <ListItem>Click _Save MCP configuration_</ListItem>
</UnorderedList>
...
<UnorderedList>
  <ListItem>Open the Command Palette</ListItem>
  <ListItem>Select "Copilot: Open MCP Settings"</ListItem>
  <ListItem>Click on "Add custom MCP"</ListItem>
  <ListItem>It will open a JSON file with your MCP servers where you can add the following configuration:</ListItem>
</UnorderedList>
...
<UnorderedList>
  <ListItem>Replace `<TOKEN>` and `<SESSION>` with your actual Carbon MCP auth credentials</ListItem>
  <ListItem>Save and close the file</ListItem>
  <ListItem>Quit and relaunch VS Code</ListItem>
</UnorderedList>
...
Here is a summary of Carbon MCP system prompt support in common
MCP clients:
...
Add a `systemPrompt` field to your Carbon MCP server <Link inline href="#step-3-connecting-to-common-mcp-clients" onClick={(e) => {
e.preventDefault();
// activate the onboarding tab if needed
document
.getElementById("step-3-connecting-to-common-mcp-clients")
?.scrollIntoView({ behavior: "smooth" });
}}>Claude Code configuration</Link> and <Link
inline
href="#step-3-connecting-to-common-mcp-clients"
onClick={(e) => {
e.preventDefault();
// activate the onboarding tab if needed
document
.getElementById("step-3-connecting-to-common-mcp-clients")
?.scrollIntoView({ behavior: "smooth" });
}}>Claude Desktop configuration</Link>
. Insert it anywhere within the carbon-mcp object (recommended placement is
after the url field for readability).
...
```markdown
{
  "mcpServers": {
    "carbon-mcp": {
      ...
      "systemPrompt": "<CARBON_MCP_SYSTEM_PROMPT>",
      "disabled": false,
      "timeout": 600,
      "headers": {
...
<UnorderedList>
  <ListItem>Replace `CARBON_MCP_SYSTEM_PROMPT` with the system prompt provided earlier in this page</ListItem>
  <ListItem>Replace `<TOKEN>` and `<SESSION>` with your actual Carbon MCP auth credentials</ListItem>
  <ListItem>Download a Claude JSON template with the `systemPrompt` attribute <a href="./files/config.json" download="config.json">here</a></ListItem>
</UnorderedList>
...
<UnorderedList>
  <ListItem>Open your project in Cursor</ListItem>
  <ListItem>Click Settings</ListItem>
  <ListItem>Click Rules and Memories</ListItem>
  <ListItem>Scroll in the dialogue to User Rules and Click Add Rule</ListItem>
  <ListItem>
    Paste the <Link
      inline
      href="#carbon-mcp-client-system-prompt"
      onClick={(e) => {
        e.preventDefault();
        // activate the onboarding tab if needed
        document
          .getElementById("carbon-mcp-client-system-prompt")
          ?.scrollIntoView({ behavior: "smooth" });
      }}>
      Carbon MCP system prompt
    </Link>into the `Workspace Rules` editor
  </ListItem>
  <ListItem>
